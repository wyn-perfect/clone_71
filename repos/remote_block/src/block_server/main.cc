...
{   
    ...
    Block_demo::Test block_handler;

	Terminal::Connection terminal;
	Signal_handler<Main> read_avail;
	char net_read_buffer[READ_BUF_SIZE];

    char read_buffer[4*1024*1024];
	char write_buffer[4*1024*1024];
...
"--- Terminal echo test started, now you can type characters to be echoed. ---\n" };
	...
	{	
		size_t num_bytes = terminal.read(net_read_buffer, READ_BUF_SIZE);
		...
		for (; info_tail < num_bytes; info_tail++) {
			if (net_read_buffer[info_tail] == '\0' || net_read_buffer[info_tail] == '\n') {
				info_len = info_tail - info_head;
				// terminal.write("\n", 1); 
				// Genode::log("info_len is ", info_len);
				// WE CAN PROCESS THE BACK INFO HERE

                // Fake logic: if start with even, then block test
                if (net_read_buffer[info_head] % 2 == 0){
                    block_handler.basic_block_test();
                }

				terminal.write(net_read_buffer + info_head, info_len + 1);
				info_head = info_tail + 1;
				info_cnt += 1;
			}
			// terminal.write(&net_read_buffer[i], 1);
		}
		// net_read_buffer[carriage_index] = '\0';

		// Genode::log("info_cnt is ", info_cnt);

		// terminal.write(net_read_buffer, carriage_index);
		// Genode::log(carriage_index);
		// terminal.write("\n", 2);
	}

	Main(Env &env) :_env(env),
                    block_handler(_env, _heap, read_buffer, write_buffer),
                    terminal(env),
